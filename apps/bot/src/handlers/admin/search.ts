...
async function requireAdmin(ctx: { from: { id: number; username?: string }; answerCbQuery: (text?: string) => Promise<unknown> }): Promise<boolean> {
...
export function registerAdminSearchHandlers({ bot: b }: HandlerContext): void {
